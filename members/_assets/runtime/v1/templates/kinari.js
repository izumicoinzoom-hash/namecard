 ...
 * 「Kinari（和紙）」テンプレ（撮影された物理名刺のスキャン演出 → 縦書きヒーロー → Story → Save & Connect）。
 * onyx.js（正解パターン）と同構造。01 Portrait のみ、
 * brightcard/preview/theme-washi.html の hero-tategaki（縦書き氏名・よみ・タグライン＋落款）を踏襲して縦書き化する。
 * 他セクション(00 intake / 02 story / 03 save / SNS FAB)・データ導出・vCard・写真（円形）・フッターC規約は onyx と同流儀。
 *
 * データは card.js（C schema）から取る。Bのwindow.CARD（tpl-washi.html等の旧データ形式）は使わない。
...
(function (global) {
  ...
  // SNSブランド別のFAB配色・アイコン（tpl-onyx.html / theme-washi.html の .chan 実装を踏襲）。
  ...
  function render(card, photoBase64, core, icons) {
    ...
    var meishi = el("div", { class: "bc-kinari-meishi" }, meishiChildren);

    var capture = el("div", { class: "bc-kinari-capture", id: "bc-kinari-namecard" }, [
    ...
    var fieldEls = [];
    var extractRows = el("div", { class: "bc-kinari-extract-rows" });
    fieldsData.forEach(function (f) {
      var fieldEl = el("div", { class: "bc-kinari-field", "data-conf": String(fieldConf[f.k] || 0) }, [
        el("span", { class: "bc-kinari-k", text: f.k }),
        el("span", { class: "bc-kinari-v", text: f.v }),
        el("span", { class: "bc-kinari-c", text: "0%" }),
      ]);
      fieldEls.push(fieldEl);
      extractRows.appendChild(fieldEl);
    });

    var extract = el("div", { class: "bc-kinari-extract", "aria-live": "polite" }, [
      el("div", { class: "bc-kinari-extract-h" }, [
        el("span", { class: "bc-kinari-t", text: "Extracted Fields" }),
        el("span", { class: "bc-kinari-dot", "aria-hidden": "true" }),
      ]),
      extractRows,
    ]);

    var intakeSection = el("section", { class: "bc-kinari-section", id: "bc-kinari-intake" }, [
      el("div", { class: "bc-kinari-ghost", "aria-hidden": "true", text: "00" }),
      el("div", { class: "bc-kinari-sec-head" }, [
        el("span", { class: "bc-kinari-chapno", text: "00" }),
        el("span", { class: "bc-kinari-sec-label", text: "Card Intake" }),
      ]),
      el("hr", { class: "bc-kinari-rule" }),
      el("p", { class: "bc-kinari-intake-copy", text: "写真付きの名刺を読み取り、名前・肩書き・連絡先を自動で抽出します。" }),
      capture,
      extract,
    ]);
    ...
    // ---------- 01 Portrait（縦書きヒーロー） ----------
    var avatarWrap = el("div", { class: "bc-kinari-avatar-wrap" });
    var img = new Image();
    ...
    img.onerror = function () {
      ...
      if (!monogramShown) {
        monogramShown = true;
        avatarWrap.appendChild(el("div", { class: "bc-kinari-monogram" }, [
    ...
    var heroChildren = [
      el("div", { class: "bc-kinari-ghost", "aria-hidden": "true", text: "01" }),
      el("div", { class: "bc-kinari-sec-head" }, [
        el("span", { class: "bc-kinari-chapno", text: "01" }),
        el("span", { class: "bc-kinari-sec-label", text: "Portrait" }),
      ]),
    ];
    if (nonEmpty(primary.role)) heroChildren.push(el("div", { class: "bc-kinari-tag", text: primary.role }));
    heroChildren.push(avatarWrap);

    // 縦書きブロック（右から タグライン → よみ → 氏名 の順にflex row-reverseで積む。
    // theme-washi.html の hero-tategaki を踏襲。名前中の空白はnbspで縦中一体を保つ）
    ...
    heroChildren.push(el("div", { class: "bc-kinari-tategaki" }, tategakiChildren));

    // 落款（seal）：氏名の頭文字（core.monogramInitial）
    heroChildren.push(el("div", { class: "bc-kinari-hero-seal-row" }, [
      el("span", { class: "bc-kinari-seal", "aria-hidden": "true", text: core.monogramInitial(name.ja) }),
    ]));

    if (nonEmpty(name.en)) heroChildren.push(el("div", { class: "bc-kinari-name-en", text: name.en }));

    var roleLine = buildRoleLine(positions);
    if (nonEmpty(roleLine)) heroChildren.push(el("div", { class: "bc-kinari-role-line", text: roleLine }));
    ...
    if (nonEmpty(card.about) || metrics.length) {
      var storyChildren = [
        el("div", { class: "bc-kinari-ghost", "aria-hidden": "true", text: "02" }),
        el("div", { class: "bc-kinari-sec-head" }, [
          el("span", { class: "bc-kinari-chapno", text: "02" }),
          el("span", { class: "bc-kinari-sec-label", text: "Story" }),
        ]),
        el("hr", { class: "bc-kinari-rule" }),
      ];
      if (nonEmpty(card.about)) storyChildren.push(el("p", { class: "bc-kinari-bio", text: card.about }));

      var proofEls = [];
      if (metrics.length) {
        var proofsGrid = el("div", { class: "bc-kinari-proofs" });
        metrics.forEach(function (m) {
          if (!m) return;
          var numAttrs = { class: "bc-kinari-num" };
          var numText;
          ...
          var proofEl = el("div", { class: "bc-kinari-proof" }, [
            numEl,
      ...
      var storySection = el("section", { class: "bc-kinari-section", id: "bc-kinari-story" }, storyChildren);
    ...
    var saveChildren = [
      el("div", { class: "bc-kinari-ghost", "aria-hidden": "true", text: "03" }),
      el("div", { class: "bc-kinari-sec-head" }, [
        el("span", { class: "bc-kinari-chapno", text: "03" }),
        el("span", { class: "bc-kinari-sec-label", text: "Save & Connect" }),
      ]),
      el("hr", { class: "bc-kinari-rule" }),
    ];
    ...
    var saveSection = el("section", { class: "bc-kinari-section", id: "bc-kinari-save" }, saveChildren);
    ...
    app.appendChild(el("div", { class: "bc-kinari-foot" }, [
      ...
      el("div", { class: "bc-kinari-powered-by", html: "Powered by <a href=\"https://withbt.com\" target=\"_blank\" rel=\"noopener\">BrightCard</a>（合同会社WBT）" }),
    ]));

    wrap.appendChild(app);

    // ---------- 収納式FAB（SNS。theme-washi.html の .chan を踏襲し3chまで） ----------
    ...
    function runIntake() {
      ...
      if (reduce) {
        fieldEls.forEach(function (f) {
          f.classList.add("bc-kinari-on");
          var c = f.querySelector(".bc-kinari-c");
          if (c) c.textContent = (f.getAttribute("data-conf") || "0") + "%";
        });
        return;
      }
      capture.classList.add("bc-kinari-scanning");
      fieldEls.forEach(function (f, i) {
        setTimeout(function () {
          f.classList.add("bc-kinari-on");
          var conf = parseInt(f.getAttribute("data-conf"), 10) || 0;
          var cEl = f.querySelector(".bc-kinari-c");
    ...
    if (typeof proofEls !== "undefined" && proofEls.length) {
      if ("IntersectionObserver" in global) {
        ...
        var storyEl = document.getElementById("bc-kinari-story");
