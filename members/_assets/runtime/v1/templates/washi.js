/**
 * BrightCard runtime v1 — templates/washi.js
 * 「Washi」テンプレ描画本体（和紙／AI生成背景＋額装二重罫フレーム・落款）。
 ...
 *
 * background: { ref: "washi/01" } または { src: "bg.jpg" } を card.js に指定すると
...
(function (global) {
  ...
  function render(card, photoBase64, core, icons) {
    ...
    // ---- Hero: AI背景 + 額装二重罫フレーム + 落款 ----
    var bgWrap = el("div", { class: "bc-washi-bg" });
    ...
    var photoWrap = el("div", { class: "bc-washi-photo" });
    var img = new Image();
    img.alt = name.ja || "";
    ...
    img.onerror = function () {
      ...
      photoWrap.appendChild(monogram);
    };
    img.src = "photo.jpg";
    photoWrap.appendChild(img);
    var frame = el("div", { class: "bc-washi-frame" }, [photoWrap]);

    var bodyChildren = [
      el("div", { class: "bc-washi-seal reveal", text: core.monogramInitial(name.ja) }),
    ];
    bodyChildren.push(el("h1", { class: "bc-washi-ja reveal reveal-d1", text: name.ja || "" }));
    if (nonEmpty(name.en)) bodyChildren.push(el("div", { class: "bc-washi-en reveal reveal-d1", text: name.en }));

    var kana = name.kana || {};
    if (nonEmpty(kana.last) || nonEmpty(kana.first)) {
      bodyChildren.push(el("div", {
        class: "bc-washi-reading reveal reveal-d1",
    ...
    if (positions.length) {
      var titlesWrap = el("div", { class: "bc-washi-titles reveal reveal-d2" });
      positions.forEach(function (p) {
        ...
        var row = el("div", { class: "bc-washi-title-item" }, [
          companySpan,
    ...
    if (nonEmpty(card.tagline)) {
      bodyChildren.push(el("p", { class: "bc-washi-tag reveal reveal-d3", text: card.tagline }));
    }

    var bodyAttrs = { class: "bc-washi-body" };
    if (background && background.overlay === "none") bodyAttrs.class += " bc-no-overlay";
    var body = el("div", bodyAttrs, bodyChildren);

    var inner = el("div", { class: "bc-washi-inner" }, [frame, body]);
  ...
  global.BrightCardTemplates.washi = render;
})(window);
