/**
 * BrightCard runtime v1 — templates/bright.js
 * 「Bright」テンプレ描画本体（暗・シネマ／AI生成背景＋角丸縦フレームの顔写真・下寄せ）。
 ...
 *
 * background: { ref: "bright/01" } または { src: "bg.jpg" } を card.js に指定すると
...
(function (global) {
  ...
  // background.ref は _assets/backgrounds/<ref>.png を指す（会員ページからの相対パス）。
  // background.src は会員フォルダ内の自前画像（同ディレクトリ相対）。
  ...
  function render(card, photoBase64, core, icons) {
    ...
    // ---- Hero: AI背景 + フレーム内フォト ----
    var bgWrap = el("div", { class: "bc-bright-bg" });
    ...
    var frame = el("div", { class: "bc-bright-frame" });
    var img = new Image();
    img.alt = name.ja || "";
    ...
    var bodyChildren = [];
    if (nonEmpty(name.en)) bodyChildren.push(el("div", { class: "bc-bright-en reveal", text: name.en }));
    bodyChildren.push(el("h1", { class: "bc-bright-ja reveal reveal-d1", text: name.ja || "" }));

    var kana = name.kana || {};
    if (nonEmpty(kana.last) || nonEmpty(kana.first)) {
      bodyChildren.push(el("div", {
        class: "bc-bright-reading reveal reveal-d1",
    ...
    if (positions.length) {
      var titlesWrap = el("div", { class: "bc-bright-titles reveal reveal-d2" });
      positions.forEach(function (p) {
        ...
        var row = el("div", { class: "bc-bright-title-item" }, [
          companySpan,
    ...
    if (nonEmpty(card.tagline)) {
      bodyChildren.push(el("p", { class: "bc-bright-tag reveal reveal-d3", text: card.tagline }));
    }

    var bodyAttrs = { class: "bc-bright-body" };
    if (background && background.overlay === "none") bodyAttrs.class += " bc-no-overlay";
    var body = el("div", bodyAttrs, bodyChildren);

    var inner = el("div", { class: "bc-bright-inner" }, [frame, body]);
  ...
  global.BrightCardTemplates.bright = render;
})(window);
