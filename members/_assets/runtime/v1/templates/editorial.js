/**
 * BrightCard runtime v1 — templates/editorial.js
 * 「Editorial」テンプレ描画本体（雑誌／AI生成背景＋左に縦フレーム・右に明朝見出し・金線）。
 ...
 *
 * background: { ref: "editorial/01" } または { src: "bg.jpg" } を card.js に指定すると
...
(function (global) {
  ...
  function render(card, photoBase64, core, icons) {
    ...
    root.classList.add("bc-editorial");
    ...
    // ---- Hero: 左AI背景+顔写真フレーム／右明朝見出し ----
    var frame = el("div", { class: "bc-editorial-frame" });
    ...
    var img = new Image();
    img.alt = name.ja || "";
    img.style.objectPosition = design.photoPosition || "center 18%";
    ...
    var bodyChildren = [
      el("div", { class: "bc-editorial-kicker reveal", text: "Business Card" }),
    ];
    bodyChildren.push(el("h1", { class: "bc-editorial-ja reveal reveal-d1", text: name.ja || "" }));
    if (nonEmpty(name.en)) bodyChildren.push(el("div", { class: "bc-editorial-en reveal reveal-d1", text: name.en }));

    var kana = name.kana || {};
    if (nonEmpty(kana.last) || nonEmpty(kana.first)) {
      bodyChildren.push(el("div", {
        class: "bc-editorial-reading reveal reveal-d1",
    ...
    if (positions.length) {
      var titlesWrap = el("div", { class: "bc-editorial-titles reveal reveal-d2" });
      positions.forEach(function (p) {
        ...
        var row = el("div", { class: "bc-editorial-title-item" }, [
          companySpan,
    ...
    if (nonEmpty(card.tagline)) {
      bodyChildren.push(el("p", { class: "bc-editorial-tag reveal reveal-d3", text: card.tagline }));
    }

    var bodyAttrs = { class: "bc-editorial-body" };
    if (background && background.overlay === "none") bodyAttrs.class += " bc-no-overlay";
    var body = el("div", bodyAttrs, bodyChildren);

    var inner = el("div", { class: "bc-editorial-inner" }, [frame, body]);
    var hero = el("section", { class: "bc-hero" }, [inner]);
  ...
  global.BrightCardTemplates.editorial = render;
})(window);
