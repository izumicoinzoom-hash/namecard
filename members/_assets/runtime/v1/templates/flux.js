 ...
 * 「Flux」テンプレ（差別化ライン再設計版・2026-07。夜光・ネオンの先鋭）。
 * 章構成: 01 Portrait（全幅ブリード＋ネオンreticle＋ロード時スキャンのヒーロー）→
 * 02 Proof（Space Monoの発光カウンター）→ 03 Story（about/philosophy/businesses/links の編集組版）→
 * 04 Reveal（紙名刺→発光ガラス銘板）→ 05 Save & Connect（vCard/共有/QR opt-in）。
 * 章番号は動的採番（空章はスキップして詰める）。
...
(function (global) {
  ...
  function render(card, photoBase64, core, icons) {
    ...
    function secHead(no, label, withRule) {
      var parts = [
        el("div", { class: "bc-flux-ghost", "aria-hidden": "true", text: no }),
        el("div", { class: "bc-flux-sec-head" }, [
          el("span", { class: "bc-flux-chapno", text: no }),
          el("span", { class: "bc-flux-sec-label", text: label }),
        ]),
      ];
      if (withRule) parts.push(el("hr", { class: "bc-flux-rule" }));
      return parts;
    }

    // ---------- 01 Portrait（全幅ブリード＋ネオンreticle＋ロード時スキャンのヒーロー） ----------
    var heroNo = chap();

    var heroPhoto = el("div", { class: "bc-flux-hero-photo" });
    var img = new Image();
    ...
    img.onerror = function () {
      ...
      if (!monogramShown) {
        monogramShown = true;
        heroPhoto.appendChild(el("div", { class: "bc-flux-hero-monogram" }, [
    ...
    // 全幅ブリードの写真に、accentネオンの reticle 四隅＋ロード時スキャンライン1回。
    var heroMedia = el("div", { class: "bc-flux-hero-media" }, [
      heroPhoto,
      ...
      el("span", { class: "bc-flux-scan bc-flux-scan-hero", "aria-hidden": "true" }),
    ]);

    var heroIdChildren = [];
    if (nonEmpty(primary.role)) {
      heroIdChildren.push(el("div", { class: "bc-flux-tag bc-flux-hi-1", text: primary.role }));
    }
    heroIdChildren.push(el("h1", { class: "bc-flux-name-ja bc-flux-hi-2", text: (name.ja || "").replace(/\s+/, " ") }));

    var kana = name.kana || {};
    if (nonEmpty(kana.last) || nonEmpty(kana.first)) {
      heroIdChildren.push(el("div", { class: "bc-flux-yomi bc-flux-hi-3", text: [kana.last, kana.first].filter(nonEmpty).join(" ") }));
    }
    if (nonEmpty(name.en)) {
      heroIdChildren.push(el("div", { class: "bc-flux-name-en bc-flux-hi-3", text: name.en }));
    }
    var roleLine = buildRoleLine(positions);
    if (nonEmpty(roleLine)) {
      heroIdChildren.push(el("div", { class: "bc-flux-role-line bc-flux-hi-4", text: roleLine }));
    }
    if (nonEmpty(card.tagline)) {
      heroIdChildren.push(el("p", { class: "bc-flux-tagline bc-flux-hi-4", text: card.tagline }));
    }

    var heroSection = el(
      "section",
      { class: "bc-flux-section bc-flux-hero", id: "bc-flux-hero" },
      secHead(heroNo, "Portrait", false).concat([
        heroMedia,
        el("div", { class: "bc-flux-hero-id" }, heroIdChildren),
      ])
    );
    app.appendChild(heroSection);

    // ---------- 02 Proof（発光カウンター。metrics 0件なら章スキップ） ----------
    var proofItems = [];
    if (metrics.length) {
      var proofNo = chap();
      var proofList = el("div", { class: "bc-flux-proof-list" });
      metrics.forEach(function (m, i) {
        if (!m) return;
        var isStatic = nonEmpty(m.static);
        var valEl = el("span", { class: "bc-flux-num-val", text: isStatic ? String(m.static) : "0" });
        var numClass = "bc-flux-num";
        if (isStatic && String(m.static).length > 5) numClass += " bc-flux-num-long";
        var numEl = el("span", { class: numClass }, [valEl]);
        if (!isStatic && nonEmpty(m.suffix)) {
          numEl.appendChild(el("span", { class: "bc-flux-num-unit", text: m.suffix }));
        }
        var item = el("div", { class: "bc-flux-p-item" + (i === 0 ? " bc-flux-p-feature" : "") }, [
          numEl,
          el("span", { class: "bc-flux-num-bar", "aria-hidden": "true" }),
      ...
      app.appendChild(el(
        "section",
        { class: "bc-flux-section", id: "bc-flux-proof" },
        secHead(proofNo, "Proof", true).concat([
          el("p", { class: "bc-flux-proof-lead", text: "数字で見る、これまでの歩み。" }),
          proofList,
        ])
      ));
    }
    ...
    if (hasStory) {
      ...
      if (nonEmpty(card.about)) {
        storyChildren.push(el("p", { class: "bc-flux-bio reveal", text: card.about }));
      }
      if (nonEmpty(philosophy.text)) {
        storyChildren.push(el("div", { class: "bc-flux-philosophy reveal" }, [
          nonEmpty(philosophy.label) ? el("div", { class: "bc-flux-phi-label", text: philosophy.label }) : null,
          el("div", { class: "bc-flux-phi-text", text: philosophy.text }),
        ]));
      }
      if (businesses.length) {
        var bizWrap = el("div", { class: "bc-flux-biz-list" });
        businesses.forEach(function (b, i) {
          var idx = (i + 1 < 10 ? "0" : "") + (i + 1) + ".";
          var tags = (Array.isArray(b.tags) ? b.tags : []).filter(nonEmpty);
          var main = el("div", { class: "bc-flux-biz-main" }, [
            el("div", { class: "bc-flux-biz-name", text: b.name || "" }),
            nonEmpty(b.role) ? el("div", { class: "bc-flux-biz-role", text: b.role }) : null,
            nonEmpty(b.desc) ? el("div", { class: "bc-flux-biz-desc", text: b.desc }) : null,
            tags.length ? el("div", { class: "bc-flux-biz-tags", text: tags.join("・") }) : null,
          ]);
          var rowChildren = [el("span", { class: "bc-flux-biz-no", text: idx }), main];
          var tag = "div";
          var attrs = { class: "bc-flux-biz-row reveal" };
          if (nonEmpty(b.url)) {
            tag = "a";
            attrs.href = absoluteUrl(b.url);
            attrs.target = "_blank";
            attrs.rel = "noopener";
            rowChildren.push(el("span", { class: "bc-flux-biz-arrow", html: icons.contact.externalLink }));
          }
      ...
      if (links.length) {
        var linkWrap = el("div", { class: "bc-flux-link-list" });
        links.forEach(function (l) {
          linkWrap.appendChild(el("a", { class: "bc-flux-link-row reveal", href: absoluteUrl(l.url), target: "_blank", rel: "noopener" }, [
            el("div", { class: "bc-flux-link-main" }, [
              el("div", { class: "bc-flux-link-label", text: l.label || displayUrl(l.url) }),
              nonEmpty(l.desc) ? el("div", { class: "bc-flux-link-desc", text: l.desc }) : null,
            ]),
            el("span", { class: "bc-flux-biz-arrow", html: icons.contact.externalLink }),
          ]));
        });
        storyChildren.push(linkWrap);
      }
      app.appendChild(el("section", { class: "bc-flux-section", id: "bc-flux-story" }, storyChildren));
    }

    // ---------- 04 Reveal（紙名刺 → 発光ガラス銘板。抽出表は存在しない） ----------
    var revealNo = chap();
    ...
    var meishi = el("div", { class: "bc-flux-meishi bc-flux-meishi-paper" }, meishiChildren);

    // 発光ガラス銘板（Before=消灯したダークカード → After=点灯した発光ガラス＋グリッド地）
    var plateChildren = [
      el("span", { class: "bc-flux-plate-accent", "aria-hidden": "true" }),
      el("div", { class: "bc-flux-plate-name", text: name.ja || "" }),
    ];
    if (nonEmpty(name.en)) plateChildren.push(el("div", { class: "bc-flux-plate-en", text: name.en.toUpperCase() }));
    var plateRole = [primary.role, primary.company].filter(nonEmpty).join(" ／ ");
    if (nonEmpty(plateRole)) plateChildren.push(el("div", { class: "bc-flux-plate-role", text: plateRole }));
    var plateRows = meishiLines.filter(function (l) { return l.mk !== "Co"; }).slice(0, 3);
    if (plateRows.length) {
      plateChildren.push(el("div", { class: "bc-flux-plate-rows" }, plateRows.map(function (l) {
        return el("div", { class: "bc-flux-plate-row" }, [
          el("span", { class: "bc-flux-mk", text: l.mk }),
          el("span", { text: l.value }),
        ]);
      })));
    }
    plateChildren.push(el("div", { class: "bc-flux-plate-badge", text: "BrightCard ── 章立てデジタル名刺" }));
    var plate = el("div", { class: "bc-flux-plate" }, plateChildren);

    var stage = el("div", { class: "bc-flux-stage", id: "bc-flux-stage" }, [
      el("span", { class: "bc-flux-reticle bc-flux-reticle-tl", "aria-hidden": "true" }),
      el("span", { class: "bc-flux-reticle bc-flux-reticle-tr", "aria-hidden": "true" }),
      el("span", { class: "bc-flux-reticle bc-flux-reticle-bl", "aria-hidden": "true" }),
      el("span", { class: "bc-flux-reticle bc-flux-reticle-br", "aria-hidden": "true" }),
    ...
    var revealSection = el(
      "section",
      { class: "bc-flux-section bc-flux-reveal", id: "bc-flux-reveal" },
      secHead(revealNo, "Reveal", true).concat([
        el("p", { class: "bc-flux-reveal-copy", text: nonEmpty(revealCfg.copy) ? revealCfg.copy : "1枚の紙の名刺から、このページは生まれました。" }),
        stage,
        el("div", { class: "bc-flux-reveal-cap reveal", text: nonEmpty(revealCfg.caption) ? revealCfg.caption : "紙の名刺が、章立ての一冊に。" }),
      ])
    );
    ...
    app.appendChild(el("section", { class: "bc-flux-section", id: "bc-flux-save" }, saveChildren));
    ...
    app.appendChild(el("div", { class: "bc-flux-foot" }, [
      ...
      el("div", { class: "bc-flux-powered-by", html: "Powered by <a href=\"https://withbt.com/card/\" target=\"_blank\" rel=\"noopener\">BrightCard</a>（合同会社WBT）" }),
    ]));
    ...
    function runProofs() {
      ...
      proofItems.forEach(function (p, i) {
        if (reduce) {
          p.item.classList.add("bc-flux-p-on");
    ...
    if (proofItems.length) {
      if ("IntersectionObserver" in global && !reduce) {
        ...
        var proofSectionEl = document.getElementById("bc-flux-proof");
    ...
    // 04 Reveal: IO発火（1回のみ）→ スキャン → 紙が退く(500ms) → 銘板が点灯する(900ms)
    ...
    function runReveal() {
      ...
      if (reduce) {
        ...
        meishi.classList.add("bc-flux-paper-out");
        plate.classList.add("bc-flux-plate-on");
        return;
      }
      stage.classList.add("bc-flux-scanning");
      setTimeout(function () { meishi.classList.add("bc-flux-paper-out"); }, 500);
      setTimeout(function () { plate.classList.add("bc-flux-plate-on"); }, 900);
    }
    if ("IntersectionObserver" in global && !reduce) {
