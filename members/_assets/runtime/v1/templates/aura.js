/**
 * BrightCard runtime v1 — templates/aura.js
 * 「Aura」テンプレ描画本体（柔光ボケ／AI生成背景＋円フレームの顔写真・中央対称）。
 ...
 *
 * background: { ref: "aura/01" } または { src: "bg.jpg" } を card.js に指定すると
...
(function (global) {
  ...
  function render(card, photoBase64, core, icons) {
    ...
    // ---- Hero: AI背景 + 円フレーム中央対称 ----
    var bgWrap = el("div", { class: "bc-aura-bg" });
    ...
    var frame = el("div", { class: "bc-aura-frame" });
    var img = new Image();
    img.alt = name.ja || "";
    ...
    var bodyChildren = [];
    if (nonEmpty(name.en)) bodyChildren.push(el("div", { class: "bc-aura-en reveal", text: name.en }));
    bodyChildren.push(el("h1", { class: "bc-aura-ja reveal reveal-d1", text: name.ja || "" }));

    var kana = name.kana || {};
    if (nonEmpty(kana.last) || nonEmpty(kana.first)) {
      bodyChildren.push(el("div", {
        class: "bc-aura-reading reveal reveal-d1",
    ...
    if (positions.length) {
      var titlesWrap = el("div", { class: "bc-aura-titles reveal reveal-d2" });
      positions.forEach(function (p) {
        ...
        var row = el("div", { class: "bc-aura-title-item" }, [
          companySpan,
    ...
    if (nonEmpty(card.tagline)) {
      bodyChildren.push(el("p", { class: "bc-aura-tag reveal reveal-d3", text: card.tagline }));
    }

    var bodyAttrs = { class: "bc-aura-body" };
    if (background && background.overlay === "none") bodyAttrs.class += " bc-no-overlay";
    var body = el("div", bodyAttrs, bodyChildren);

    var inner = el("div", { class: "bc-aura-inner" }, [frame, body]);
  ...
  global.BrightCardTemplates.aura = render;
})(window);
