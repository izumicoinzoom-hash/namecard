...
## 6. 課金・料金設計（売り切り＋都度再課金）※2026-07-13 市場調査反映・価格確定
...
### 6.1 本体価格 ★確定: 素材別「全部入り」B方式（2026-07-13 改定）
物理NFCカード（タップで名刺ページが開く）を同梱し、素材別に「デジタル制作＋カード込み」の一括価格で販売。デジタル制作費は当初¥9,800→**半額¥4,900**（布教価格）に引き下げ、カード原価（実購入で判明）を上乗せ。

| 商品（税込・全部入り） | 価格 | カード原価/枚 | 素材 |
|---|---|---|---|
| **黒（無地PVC）込み** | **¥4,900** | ¥136 | NTAG215 黒無地 |
| **木（チェリーウッド）込み** | **¥5,400** | ¥296 | NTAG215 木製 |
| **アルミ（ヘアラインシルバー）込み** | **¥5,900** | ¥1,000 | NTAG215 メタリック |

- カード原価は清田さんのAmazon実購入で確定（2026-03-23注文・出品者eREC: 木¥1,480/5枚=¥296、黒¥680/5枚=¥136）。アルミは国内メタリックカード¥1,000/枚（Amazon実勢・【国内製造】ヘアラインシルバー）。
- **「黒」は黒メタルではなく黒無地PVC**（清田さん確認済み）。
- カードはNTAG215ブランク（無地）。NFCをURLに書き込むだけの「タップ点」。**名入れ印刷はしない**（原価・工数を抑える）。将来オプションで名入れ版を上位に置く余地あり。
- ポジション: ¥4,900〜5,900は量産NFC名刺市場(¥2,980〜4,980)とほぼ同価格帯に着地。**同じ価格で「専用デザイン＋物理カード」**を出す＝量産品への明確な差別化。
- 原価: カード¥136〜1,000＋制作工数（2枚目以降15〜30分≒¥2,000前後）。¥4,900でも粗利確保。

### 6.2 更新の都度課金 ★全額半額（2026-07-13改定）
| 種別 | 例 | 価格（税込） |
|---|---|---|
| 軽微作業（テキスト修正） | 電話・住所・役職・タグ（同一セクション5項目まで） | **¥1,650** |
| 写真差し替え | photo再生成＋og:image確認 | **¥2,200** |
| 構成変更 | セクション追加・文章書き直し | **¥2,750** |
| デザイン全面変更 | テンプレ切替＋全体QA | **¥3,300** |

本体引き下げに合わせ更新料も全て半額（旧: ¥3,300/4,400/5,500/6,600）。最低受注¥1,650。
**初回1ヶ月以内の軽微修正1回は無償**（直接競合が更新無料のため初売りの摩擦を下げる）。それ以降は有償。**WBT側瑕疵（表示不具合・リンク切れ）は無償／会員都合の変更はすべて有償**。この線引きを納品案内に明記。
...
- 単発（主軸）: 初期のみ ¥4,900〜5,900（素材別・カード込み）。以後は都度課金
